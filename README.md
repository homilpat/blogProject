...
> [!NOTE]
> **작업 중인 프로젝트입니다.** 이 `main` 브랜치는 안정화된 기본 파이프라인입니다. 50문항 평가셋 기반 검색·생성 경로 개선은 [`future-work/rag-search-pipeline`](https://github.com/homilpat/blogProject/tree/future-work/rag-search-pipeline) 브랜치에서 진행 중이며, 내용은 아래 [작업 중인 개선](#작업-중인-개선) 절에 정리했습니다.
...
| 현재 상태 | `main`: 게시글 CRUD·자동 색인·근거 기반 질의응답·대화 저장·인증/권한·이미지 관리 구현 / 작업 중: 평가셋 기반 검색·생성 경로 개선 (`future-work/rag-search-pipeline`) |
...
> `future-work/rag-search-pipeline` 브랜치에서 진행 중이며 아직 `main`에 병합하지 않았습니다. 아래 수치는 해당 브랜치에서 2026-09-04에 golden 50문항으로 측정한 결과입니다.

### 검색 전략을 평가셋으로 선택

대표 기술 질문 40개, 문서에 근거가 없는 질문 5개, 확인 질문·일반 대화 5개로 golden set을 만들고 각 문항에 기대 의도·정답 출처 ID·필수 개념을 붙였습니다.
...
Cross-Encoder는 품질이 오르지 않으면서 검색 지연만 약 170배 늘어 채택하지 않았습니다. 코사인 유사도용 최소 점수를 범위가 다른 RRF·Cross-Encoder 점수에 그대로 적용해 hybrid Hit@K가 `0.20`으로 떨어지던 버그도 점수 체계별 임계값 분리로 수정했습니다. 실패 문항은 회귀 목록에 자동 등록되며 현재 18건입니다.

### 근거 구조화와 최종 생성의 모델 분리

- **Qwen3 8B (Evidence Structurer):** 필요한 근거와 원문에서 확인되는 사실을 `FACT / INFERENCE / PARTIAL` JSON으로만 구조화합니다.
- **Gemma 4 E4B (Answer Renderer):** 구조화된 근거와 선택된 원문만 받아 최종 답변을 작성합니다.
- **결정론적 인용 검증:** 마지막 검증을 LLM에 다시 맡기지 않고 코드가 인용 번호와 출처 존재 여부를 확인합니다.

| 생성 경로 | 통과율 | 정확성 | 근거 블록 | 인용 유효성 | 평균 지연 |
|---|---:|---:|---:|---:|---:|
| Qwen 직접 | 0.820 | 0.870 | **0.893** | 1.000 | 21.3초 |
| Qwen→Gemma 계층형 | **0.940** | **0.938** | 0.810 | 1.000 | 33.4초 |
| **Auto 라우팅 (채택)** | 0.880 | 0.896 | 0.840 | 1.000 | 25.8초 |

계층형은 통과율과 정확성이 높지만 근거 블록 비율은 Qwen 단독보다 낮아, 질문 구조와 근거 종합 필요성으로 경로를 자동 선택하는 Auto를 품질과 응답 시간의 절충안으로 택했습니다. Auto의 정책 기준 경로 일치율은 74%이며, 이는 최적 경로의 정답률이 아니라 현재 라우팅 정책과의 일치율입니다. 경로별 문항당 1회 실행 결과이므로 일반적 품질 우위를 확정하려면 반복 실행이 더 필요합니다.

### 브랜치에서 함께 추가 중인 기능

- 관리자용 RAG Evaluation 대시보드와 요청 단계별 trace
- 내부 근거가 없을 때만 동작하는 자체 호스팅 SearXNG 웹 보강
- 내부 자료 우선 근거 기반 학습 방향 추천
...
- `main`에는 검색 결과 재순위화와 정량 평가 데이터셋이 아직 병합되지 않았습니다. 평가셋 구축과 Cross-Encoder 비교는 작업 브랜치에서 진행 중입니다.
...
2. 검색 품질 평가셋 기반 검색·생성 경로 개선 병합 (작업 중)
