...
class AnswerRenderer:
    ...
    def generate_draft(
        ...
        system_prompt = (
            ...
            "원문 문장, 원문 제목, 검색 점수, 출처 목록을 답변 본문에 복사하지 마세요. 원문의 의미만 보존해 자연스러운 한국어로 재서술하고 원문 자체는 별도 근거 카드에 맡기세요. "
            "'[근거 N] 제목: 원문' 형식이나 '검색된 원문을 확인하세요' 같은 안내문을 답변 대신 출력하지 마세요. [근거 N]은 재서술한 주장 뒤의 짧은 출처 표시에만 사용하세요. "
    ...
    def generate_final(
        ...
        system_prompt = (
            ...
            "원문 문장과 제목을 답변 본문에 그대로 복사하거나 출처별로 나열하지 마세요. 원문의 의미를 자연스럽게 재서술하고 원문 자체는 별도 근거 카드에서만 보여줍니다. "
            "'[근거 N] 제목: 원문' 형식이나 '검색된 원문을 확인하세요' 같은 안내문으로 답변을 대신하지 마세요. "
